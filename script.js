...
(function initCursor() {
  ...
  document.querySelectorAll('a, button, .filter-btn, .portfolio-item, .service-card, .blog-card, .motion-card').forEach(el => {
...
(function initVideoSection() {
  const frames = document.querySelectorAll('.featured-video-frame');
  if (!frames.length) return;

  frames.forEach(frame => {
    const playBtn = frame.querySelector('.video-play-button');
    if (!playBtn) return;

    playBtn.addEventListener('click', () => {
      const video = frame.querySelector('.featured-video-preview');
      if (video) {
        ...
        video.muted = false;
        video.currentTime = 0;
        frame.querySelector('.featured-video-overlay')?.remove();
        video.play().catch(() => {
          showToast('Use the video controls to play this story.');
        });
        return;
      }

      const src = frame.dataset.videoSrc;
      if (!src || frame.classList.contains('is-playing')) return;

      const iframe = document.createElement('iframe');
      iframe.src = src;
      iframe.title = 'Visva Events featured video';
      iframe.allow = 'accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share';
      iframe.allowFullscreen = true;

      frame.classList.add('is-playing');
      frame.appendChild(iframe);
      frame.querySelector('.featured-video-preview')?.remove();
      frame.querySelector('.featured-video-overlay')?.remove();
    });
  });
})();
...
// G+M Gallery
(function initGmGallery() {
  const galleryPhotos = [
    { src: 'UNEDITED/DSC_9870.JPG', alt: 'G+M celebration photo 1' },
    { src: 'UNEDITED/DSC_9843.JPG', alt: 'G+M celebration photo 2' },
    { src: 'UNEDITED/DSC_9774.JPG', alt: 'G+M celebration photo 3' },
    { src: 'UNEDITED/DSC_0003.JPG', alt: 'G+M celebration photo 4' },
  ];

  const trigger = document.querySelector('[data-gallery-open="gm"]');
  const modal = document.getElementById('gmGallery');
  const image = document.getElementById('gmGalleryImage');
  const count = document.getElementById('gmGalleryCount');
  const thumbs = document.getElementById('gmGalleryThumbs');
  const prevBtn = modal?.querySelector('.gallery-prev');
  const nextBtn = modal?.querySelector('.gallery-next');
  if (!trigger || !modal || !image || !count || !thumbs || !prevBtn || !nextBtn) return;
  ...
  function render(index) {
    ...
    image.src = photo.src;
    image.alt = photo.alt;
    count.textContent = `Photo ${currentIndex + 1} of ${galleryPhotos.length}`;
  ...
  galleryPhotos.forEach((photo, index) => {
    ...
    thumb.type = 'button';
    thumb.setAttribute('aria-label', `Open G+M photo ${index + 1}`);
